...
class Slide:
    ...
    def check_is_img_path(img, img_type, resize):
        if img.startswith('http'):
            ...
            if img_res.status_code == 200:
                ...
                image = cv2.imdecode(image, cv2.IMREAD_COLOR)  # type: ignore
                if resize:
                    image = cv2.resize(image, dsize=resize)  # type: ignore
                cv2.imwrite(img_path, image)  # type: ignore
                return img_path
            else:
    ...
    def clear_white(img):
        ...
        img = cv2.imread(img)  # type: ignore
    ...
    def template_match(self, tpl, target):
        th, tw = tpl.shape[:2]
        result = cv2.matchTemplate(target, tpl, cv2.TM_CCOEFF_NORMED)  # type: ignore
        ...
        # (0,0,255)：矩形边框颜色
        # 1：矩形边框大小
        cv2.rectangle(target, tl, br, (0, 0, 255), 2)  # type: ignore
        cv2.imwrite(self.out, target)  # type: ignore
        return tl[0]

    @staticmethod
    def image_edge_detection(img):
        edges = cv2.Canny(img, 100, 200)  # type: ignore
        return edges

    def discern(self):
        img1 = self.clear_white(self.gap)
        img1 = cv2.cvtColor(img1, cv2.COLOR_RGB2GRAY)  # type: ignore
        slide = self.image_edge_detection(img1)

        back = cv2.imread(self.bg, cv2.COLOR_RGB2GRAY)  # type: ignore
        back = self.image_edge_detection(back)

        slide_pic = cv2.cvtColor(slide, cv2.COLOR_GRAY2RGB)  # type: ignore
        back_pic = cv2.cvtColor(back, cv2.COLOR_GRAY2RGB)  # type: ignore
